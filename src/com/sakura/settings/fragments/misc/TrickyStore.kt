...
class TrickyStore : SettingsPreferenceFragment() {
    ...
    private fun refreshStatus() {
        val keyboxExists = !Settings.System.getString(
            requireContext().contentResolver, KEYBOX_KEY
        ).isNullOrEmpty()

        val targetContent = Settings.System.getString(requireContext().contentResolver, TARGET_KEY)
    ...
    private fun readTargetPackages(): Set<String> {
        val content = Settings.System.getString(requireContext().contentResolver, TARGET_KEY)
            ?: return emptySet()
